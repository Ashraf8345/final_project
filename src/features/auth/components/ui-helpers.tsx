...
export function ErrorAlert({ message }: { message: string | null }) {
  ...
  return (
    <div
      role="alert"
      className="flex items-start gap-3 rounded-lg border border-red-500/20 bg-red-950/20 px-4 py-3 text-xs text-red-400 shadow-sm leading-normal animate-in fade-in duration-200"
    >
...
export function SuccessAlert({ message }: { message: string | null }) {
  ...
  return (
    <div
      role="status"
      className="flex items-start gap-3 rounded-lg border border-emerald-500/20 bg-emerald-950/20 px-4 py-3 text-xs text-emerald-400 shadow-sm leading-normal animate-in fade-in duration-200"
    >
      <svg className="h-4 w-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
